...
def select_eps_greedy_action(table, state, n_actions):
	...
	'''
	value, action = select_greedy_action(table, state, n_actions)
...
def select_greedy_action(table, state, n_actions):
	'''
...
def Q_learning(table, obs0, obs1, reward, action, n_actions):
	'''
	...
	best_value, _ = select_greedy_action(table, obs1, n_actions)

	# Update Q(obs0,action)
	table[(obs0, action)] += LEARNING_RATE * (reward + GAMMA * best_value - table[(obs0, action)])
...
def test_game(env, table, n_actions):
	...
	for _ in range(TEST_EPISODES):
		...
		while True:
			# Act greedly
			_, best_action = select_greedy_action(table, obs, n_actions)
...
while games_count < MAX_GAMES:
	...
	# Update the Q-table
	Q_learning(table, obs, next_obs, reward, action, n_actions)
